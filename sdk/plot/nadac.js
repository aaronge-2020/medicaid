...
async function getAllNdcObjs() {
  ...
  for (let i = 0; i < distributions.length; i += 4) {
    ...
    const response = await getDatastoreQuerySql(
      `[SELECT ndc,ndc_description FROM ${distributions[i]}]`
    );
...
async function getNdcFromMed(med) {
  ...
  throw new Error(
    "Please provide a medicine that is included in the medicaid dataset."
  );
}
...
async function getMedNames(medicine) {
  ...
  return medList.filter(
    (med) => med.split(" ")[0] === `${baseMedName.toUpperCase()}`
  );
}

async function getNameFromNDC(ndc) {
  const ndcObjMap = await getAllNdcObjs();

  for (const [key, value] of ndcObjMap.entries()) {
    if (value.has(ndc)) return key;
  }
  throw new Error("Please provide a valid NDC.");
}

async function getMedData(
  items,
  filter = "ndc",
  dataVariables = ["as_of_date", "nadac_per_unit"]
) {
  ...
  if (filter === "ndc") {
    ...
    items.forEach((item) => {
      if (!ndcs.has(item))
        throw new Error(
          "This NDC is not contained within the Medicaid Dataset."
        );
    });
  }
  await updateNadac();
...
async function getMedPlotData(
  meds,
  filter,
  axis = { xAxis: "as_of_date", yAxis: "nadac_per_unit" }
) {
  const medList = Array.isArray(meds) ? meds : [meds];
  const medData = await getMedData(medList, filter, Object.values(axis));
  medData.sort(
    (a, b) => convertDate(a[axis.xAxis]) - convertDate(b[axis.xAxis])
  );
...
async function plotNadacNdc(ndcs, layout, div, axis) {
  ...
  const data = await Promise.all(
    medList.map((med) => getMedPlotData(med, "ndc", axis))
  );
...
async function plotNadacMed(meds, layout, div, axis) {
  ...
  const medList = Array.isArray(meds) ? meds : [meds];
  const data = await Promise.all(
    medList.map((med) => getMedPlotData(med, "ndc_description", axis))
  );
  return plot(data, layout, "line", div);
}
...
async function preImport() {
  let datasets = (await getDatasetByKeyword("nadac", false)).filter((r) =>
    r.title.includes("(National Average Drug Acquisition Cost)")
  );
  datasets = datasets.sort((a, b) => a.title.localeCompare(b.title));
  distributions = await Promise.all(
    datasets.map((r) => {
      return convertDatasetToDistributionId(r.identifier);
    })
  );
  await endpointStore.removeItem(
    `metastore/schemas/dataset/items/${
      datasets[datasets.length - 1].identifier
    }`
  );
}
